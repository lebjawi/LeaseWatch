...
async def scrape_columns() -> List[FloorPlan]:
    """Main function to scrape The Columns at Lake Ridge apartment data"""
    logger.scraping_start("The Columns at Lake Ridge")
    
    browser = None
    
    try:
        async with async_playwright() as p:
            logger.browser_launch("Columns")
            ...
            url = 'https://www.thecolumnsatlakeridge.com/apartments/ga/dunwoody/floor-plans#/'
            ...
            floor_plans = await extract_columns_floor_plans(page)
            
            logger.scraping_complete("The Columns at Lake Ridge", len(floor_plans))
            return floor_plans
    
    except Exception as error:
        error_msg = str(error) if error else "Unknown error"
        logger.scraping_error("The Columns at Lake Ridge", error_msg)
        return []
    
    finally:
        if browser:
            await browser.close()
            logger.browser_close("Columns")


async def extract_columns_floor_plans(page: Page) -> List[FloorPlan]:
    """Extract floor plan data from The Columns at Lake Ridge page"""
    try:
        # Wait for floor plan content to load
        logger.info('⏳ Waiting for Columns floor plans to load...')
        ...
        # Extra wait for dynamic content (especially for SPA sites)
        await page.wait_for_timeout(3000)
        
        logger.info('🔍 Extracting Columns floor plan data...')
        
        # Extract floor plan data using page.evaluate
        floor_plans_data = await page.evaluate("""
            () => {
                // Try multiple selectors for The Columns website
                ...
                floorPlanCards.forEach((card, index) => {
                    try {
                        ...
                        console.log('Error processing Columns floor plan card:', error);
                    }
                });
        ...
        logger.info(f'📊 Found {len(floor_plans_data)} potential Columns floor plan cards')
        ...
        for data in floor_plans_data:
            try:
                ...
                floor_plan = FloorPlan(
                    ...
                    property_name="The Columns at Lake Ridge",
                    property_url="https://www.thecolumnsatlakeridge.com/apartments/ga/dunwoody/floor-plans",
                    unit_type="apartment"
                )
        ...
        logger.info(f'✅ Successfully processed {len(floor_plans)} valid Columns floor plans')
        return floor_plans
    
    except Exception as error:
        logger.error(f'❌ Error extracting Columns floor plans: {str(error)}')
        return []
